...
extension = "deux_lignes_mur_1m25_rp_moy30_convec"
...
def est_sur_lignes_sol_mur(x,y,z):
    return ( (z==0) and (int(Ny/3)-2 <=y and y<= int(Ny/3)+2 or 2*int(Ny/3)-2 <=y and y<= 2*int(Ny/3)+2)) \
        or (((y<=1) or (y>=Ny-2)) and (int(Nz/2)-2 <=z and z<= int(Nz/2)+2))
...
#while t[int(Nx/2),int(Ny/2),int(Nz/2)]==T0:
...
ax.set_title("Distribution de température en RP")
ax.set_xlabel('X')
ax.set_ylabel('Y')
ax.set_zlabel('Z')
save(t,extension,6)
